...
data[one_hot_feature] = data[one_hot_feature].fillna(data[one_hot_feature].mean())#空值用平均值填充填充

for feature in one_hot_feature:
    if feature != 'videoplayseconds':
        data[feature] = LabelEncoder().fit_transform(data[feature].apply(str))#标签编码、对数据进行转换

dense_features = ['videoplayseconds']
mms = MinMaxScaler(feature_range=(0, 1))#数据归一化
data[dense_features] = mms.fit_transform(data[dense_features])#对密集特征进行转换
